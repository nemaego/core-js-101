 ...
 *   ''      => 0
 */
function getStringLength(yourString) {
  return yourString.length;
}
...
function encodeToRot13(str) {
  ...
  const caesarAlphabet = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';

  return str.split('').map((char) => {
    const index = englishAlphabet.indexOf(char);
    return index !== -1 ? caesarAlphabet[index] : char;
  }).join('');
}
